...
- **Supabase → Scalingo** (in progress, see §7) — Postgres database, currently still live on Supabase, actively being migrated to a self-hosted Postgres + PostgREST setup on Scalingo. The app's own code (`lib/supabase/*`) is unchanged either way — same `@supabase/supabase-js` client, just pointed at a different URL depending on which backend is live. Supabase Storage remains the file-storage backend regardless of which database is live (only `exercise-media` uses it — see the HDS memory note for why the other two storage use cases were dropped instead of migrated).
