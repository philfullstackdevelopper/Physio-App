...
- **Exercise illustrations live in the repo, not in any storage service**: `public/exercise-illustrations/<slug>/` (the vectorised drawings, several frames per exercise), `public/exercise-illustrations-draft/`, `public/body-parts/`, mapped by `lib/exercise/illustrationMap.ts` — so they deploy with the code on Scalingo and need nothing else.
- **Supabase Storage** — still wired as the file-storage backend for the single remaining bucket `exercise-media` (exercise demo *videos* — none uploaded yet, the bucket is empty as of 2026-09-13, so Supabase currently holds nothing the app needs) (exercise demo videos, not health data). Philippe wants this off Supabase too; Scalingo has no persistent file storage of its own (containers are ephemeral), so the target is **Outscale OOS** (S3-compatible, Scalingo's own recommended pairing) — `lib/storage/s3.ts` is already written and dormant, waiting only on an Outscale account + access keys that Philippe has to create himself. Until then, `NEXT_PUBLIC_SUPABASE_STORAGE_URL` / `SUPABASE_STORAGE_URL` keep Storage pointed at Supabase separately from the database URL (`lib/supabase/client.ts` `createStorageClient()`, `lib/supabase/admin.ts`) — a single `supabase-js` client can't serve both, since PostgREST has no Storage API.
- **Stripe** (Connect) — per-patient billing, see §4.
- **Scalingo** — hosting, decided 2026-09-13. The `easyphysio` app (region `osc-fr1`, project `default`) deploys from the `main` branch on GitHub via Scalingo's GitHub integration — if a push to `main` doesn't trigger a build, run `scalingo --region osc-fr1 --app easyphysio integration-link-manual-deploy main`. The PostgREST layer is a second app, `easyphysio-postgrest` (see §7). **Vercel is abandoned** (Philippe, 2026-09-13): the old `physio-app-topaz.vercel.app` deployment is no longer maintained and its config is stale (still on Supabase, missing `SCALINGO_DATABASE_URL`) — it should be disconnected/deleted in the Vercel dashboard so it stops auto-deploying `main` and nobody creates data there. Don't reference Vercel in new work.
- **GitHub** — repo at `philfullstackdevelopper/Physio-App`.

Environment variables live in `.env.local` locally and in the Scalingo app's environment (`scalingo --region osc-fr1 --app easyphysio env` / `env-set`) for the deployed app. Never commit `.env.local` to git, and never print or read its contents unless the user asks.
